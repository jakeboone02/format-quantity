...
- `formatQuantity` and `formatRomanNumerals` now accept `bigint` values.
...
- UMD build renamed `dist/format-quantity.umd.min.js`.
...
- UMD build sets `FormatQuantity` global variable when run in a browser (`window` is defined).
